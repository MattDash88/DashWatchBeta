  ...
  render() {
    ...
    if (financial_data[0] == "N/A") { // If there is no funding data, no subtabs
      ...
      if (this.state.reportTab == "") { // Open first subtab as default
        this.setState({ reportTab: this.props.financial_data[0].date_range })
      }
      ...
      reportdata = (
        ...
        "Inactive"} onClick={this.displayTab}>{post.date_range}</button>
        )
      )
    }
...
class ReportTabContent extends React.Component {
  ...
  render() {
    ...
    const { // Declare financial elements used in the subtab 
      ...
      return (
        <div className="reportTab" value={report_tab == date_range ? "Active" :
          "Inactive"}>
